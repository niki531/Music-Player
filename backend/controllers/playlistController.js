...
export const getPlaylists = async (ctx) => {
  try {
    const playlists = await PlaylistIndex.find({ public: true }).toArray();
    ctx.body = {
      data: playlists,
...
export const createPlaylist = async (ctx) => {
  const { uid, username } = ctx.state.user;
...
export const removeTrackFromPlaylist = async (ctx) => {
  const { uid } = ctx.state.user;
  const playlistId = ctx.params.pid;
  const trackId = ctx.query.trackId;
  ...
  try {
    ...
    const playlistCollection = playlistConnection.collection(`p_${playlistId}`);
    ...
    await decrementCounter(`p_${playlistId}_order`);
