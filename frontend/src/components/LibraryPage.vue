...
export default {
  ...
  methods: {
    async fetchLibrary(type) {
      ...
      try {
        const response = await apiClient.get('/library', { params: { type } });
    ...
    async getAlbumData(pid) {
      try {
        const response = await apiClient.get(`/album/${pid}`);
    ...
    async playTrack(track, index) {
      try {
        const trackResponse = await apiClient.get(`/stream/${track.id}`, { responseType: 'blob' });
        ...
        const updatedTracks = await Promise.all(this.library.map(async (t) => {
          const tResponse = await apiClient.get(`/stream/${t.id}`, { responseType: 'blob' });
