...
export default {
  ...
  async created() {
    ...
    try {
      const playlistResponse = await apiClient.get(`/playlist/${pid}`);
      this.playlist = playlistResponse.data.data;    
      ...
      const trackDetails = await Promise.all(
        this.playlist.tracks.map(async (track) => {
          const trackResponse = await apiClient.get(`/track/${track.tid}`);
  ...
  methods: {
    async fetchPlaylistCover(pid) {
      try {
        const response = await apiClient.get(`/image/${pid}`, { responseType: 'blob' });
        this.playlistCover = URL.createObjectURL(response.data);
      } catch (error) {
        console.error(`Error fetching playlist cover for ${pid}:`, error);
        try {
          const response = await apiClient.get(`/image/default-playlist-cover`, { responseType: 'blob' });
          this.playlistCover = URL.createObjectURL(response.data);
        } catch (defaultError) {
    ...
    async playTrack(track, index) {
      const { pid } = this.$route.params;
      try {
        const trackResponse = await apiClient.get(`/stream/${track.track_id}`, { responseType: 'blob' });
        ...
        const updatedTracks = await Promise.all(this.tracks.map(async (t) => {
          const tResponse = await apiClient.get(`/stream/${t.track_id}`, { responseType: 'blob' });
    ...
    async addTrackToLibrary(trackId) {
      try {
        await apiClient.post('/track/library', { type: 'track', id: trackId });
    ...
    async addPlaylistToLibrary(pid) {
      try {
        await apiClient.post('/track/library', { type: 'playlist', id: pid });
    ...
    async getTrackData(tid) {
      try {
        const response = await apiClient.get(`/track/${tid}`);
    ...
    async removeFromPlaylist(tid) {
      const { pid } = this.$route.params;
      try {
        await apiClient.delete(`/playlist/${pid}/track`, {
          params: {
            trackId: tid
          }
        });
    ...
    async deletePlaylist() {
      const { pid } = this.$route.params;
      try {
        const response = await apiClient.delete(`/playlist/${pid}`);
        const response2 = await apiClient.delete(`/library/playlist/${pid}`);
