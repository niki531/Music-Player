  ...
  methods: {
    ...
    async fetchLibrary(type) {
      ...
      try {
        ...
        const response = await apiClient.get('/user-library-service/library', {
          params: { uid, type }
        });
    ...
    async getAlbumData(pid) {
      try {
        ...
        console.log('album is',response.data);
    ...
    async playTrack(track, index) {
      try {
        ...
        // 将轨道信息传递到 appPage 中，而不再直接发起 API 请求
        this.$root.$emit('play-track', {
          src: '', // 不再直接获取音频流
          title: trackData.name,
          subtitle: this.cleanAuthor(trackData.artist),
          tid: track.id,
          album_id: trackData.album_id,
          albumCover: coverUrl,
        }, this.library, index, coverUrl);
      } catch (error) {
        console.error('Error fetching track data:', error);
      }
    },
